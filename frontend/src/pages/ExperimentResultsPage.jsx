  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-900">RecoverAI vs. Baseline</h2>
        <button
          onClick={handleRun}
          disabled={running}
      ...
      {error && !result && (
        <div className="p-3 rounded-md bg-amber-50 text-amber-700 text-sm">{error}</div>
      )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {Object.entries(result.recoverAI.failureTypeBreakdown || {}).map(([reason, b]) => (
                  <tr key={reason}>
                    <td className="py-2">{formatFailureReason(reason)}</td>
                    <td className="py-2 text-right">{b.count}</td>
                    <td className="py-2 text-right">{b.avgAttempts}</td>
                    <td className="py-2 text-right">{b.notified}</td>
